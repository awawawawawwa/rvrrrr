...
fn full_pipeline_two_frames() {
    ...
    let msg1: ProtocolMessage = serde_json::from_str(frame1).unwrap();
    if let ProtocolMessage::Render { root } = msg1 {
        current.clear();
    ...
    let msg2: ProtocolMessage = serde_json::from_str(frame2).unwrap();
    if let ProtocolMessage::Render { root } = msg2 {
        current.clear();
        paint_tree(&mut current, &root, &clip);
        let runs = compute_diff(&previous, &current);
        assert!(!runs.is_empty());
...
fn identical_frames_produce_empty_diff() {
    let frame = r#"{"type":"render","root":{"kind":"text","layout":{"x":0,"y":0,"width":10,"height":1},"content":"Same text!","wrap":"wrap"}}"#;

    let mut buf1 = Buffer::new(20, 5);
    let mut buf2 = Buffer::new(20, 5);
    let clip = ClipRect::full(20, 5);

    let msg: ProtocolMessage = serde_json::from_str(frame).unwrap();
    if let ProtocolMessage::Render { ref root } = msg {
...
fn malformed_json_returns_parse_error() {
    ...
    let result: Result<ProtocolMessage, _> = serde_json::from_str(bad_json);
    assert!(result.is_err());
}

#[test]
fn empty_line_is_not_valid_json() {
    let result: Result<ProtocolMessage, _> = serde_json::from_str("");
    assert!(result.is_err());
}
